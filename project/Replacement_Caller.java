...
public class Replacement_Caller extends JFrame {
	...
	public static void main() {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Replacement_Caller frame = new Replacement_Caller();
	...
	public Replacement_Caller() {
		...
		spinner.setBounds(278, 230, 65, 20);
		getContentPane().add(spinner);
		
		String field[]= {"Comps","Mech","IT","Civil","EXTC"};

		JComboBox comboBox = new JComboBox(field);
		comboBox.setBounds(278, 163, 109, 20);
		contentPane.add(comboBox);
		
		JLabel lblField = new JLabel("Field?");
		lblField.setFont(new Font("Tahoma", Font.BOLD, 12));
		lblField.setBounds(78, 166, 77, 14);
		contentPane.add(lblField);

		JButton btnCheck = new JButton("Check!");
		btnCheck.setIcon(new ImageIcon(Replacement_Caller.class.getResource("/images/list (1).png")));
		...
		btnCheck.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				Replacement.main(days[n], c,field[z]);
			}
		});
		...
		label.setIcon(new ImageIcon(Replacement_Caller.class.getResource("/images/ADMIN BACKGROUND.png")));
